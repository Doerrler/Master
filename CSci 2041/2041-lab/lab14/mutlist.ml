...
let rec insert_after_m a b ml = match ml with
  | Nil -> raise Not_found
  | C mc -> if (mc.hd = a) then mc.tl <- C{hd = b; tl = mc.tl}
    else insert_after_m a b mc.tl
(*This should return type unit, I have it correctly adding the value but it isn't
of type ()*)
...
let rec exclude_m p ml = match ml with
  | Nil -> Nil
  | C mc -> if (p mc.hd) then exclude_m p mc.tl
    else C {hd = mc.hd; tl = (exclude_m p mc.tl)}
