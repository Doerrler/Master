...
let rec snooze s () = Lwt_unix.sleep s >>= fun () -> Lwt_io.printl "WAKE UP YOU NEED TO MAKE MONEY" >>= fun () -> snooze s ()

(* And your definition of pick_out*)
let rec pick_out l = Lwt_list.iter_s (fun (i,b) -> if b then Lwt_io.printl (string_of_int i) else Lwt.return ()) l
