...
module FunDict = struct
  ...
  let add (key, vl) d = fun k -> if key = k then vl else d k
  let update (key, vl) d = fun k -> if key = k then vl else d k
  let lookup k d = d k
  end
...
module DictTest(DT1: DICT)(DT2: DICT) = struct
	...
	let dict1 = List.fold_left (fun d (k,v) -> DT1.add k v d) DT1.empty ins_list in
	let dict2 = List.fold_left (fun d (k,v) -> DT2.add k v d) DT2.empty ins_list in 
	let check k d1 d2 = if (DT1.lookup k d1) = (DT2.lookup k d2) then None else Some k in
	let rec help tlst = match tlst with
		|[]->None
		|h::t -> match check h dict1 dict2 with
			|None -> help t
			|Some k -> Some k
in help testlist
  end
...
module DictUtils(S: FOLDABLEDICT) = struct
    ...
    let for_all_vals p d = TreeDict.fold (fun b (k,v) -> b && (p v)) true d
  end
